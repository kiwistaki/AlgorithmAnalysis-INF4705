function rapportGraph( dataC, dataQ, dataQR, dataQRS, dataQS, str, fx, saveName)
%on veut y/fx
%x pire cas counting sort
%x log x moyen/best quicksort
%x^2 pire quicksort
...
switch(fx)
    ...
    case 1
        %(fx = y/x)
        dataC(:,:,2) = (dataC(:,:,2) ./ dataC(:,:,1));
        dataQ(:,:,2) = (dataQ(:,:,2) ./ dataQ(:,:,1));
        dataQR(:,:,2) = (dataQR(:,:,2) ./ dataQR(:,:,1));
        dataQRS(:,:,2) = (dataQRS(:,:,2) ./ dataQRS(:,:,1));
        dataQS(:,:,2) = (dataQS(:,:,2) ./ dataQS(:,:,1));
        str = [str ' f(x) = x'];
    case 2
        %(fx = y/ x log x)
        dataC(:,:,2) = dataC(:,:,2) ./ (dataC(:,:,1) .* log2(dataC(:,:,1)));
        dataQ(:,:,2) = dataQ(:,:,2) ./ (dataQ(:,:,1) .* log2(dataQ(:,:,1)));
        dataQR(:,:,2) = dataQR(:,:,2) ./ (dataQR(:,:,1) .* log2(dataQR(:,:,1)));
        dataQRS(:,:,2) = dataQRS(:,:,2) ./ (dataQRS(:,:,1) .* log2(dataQRS(:,:,1)));
        dataQS(:,:,2) = dataQS(:,:,2) ./ (dataQS(:,:,1) .* log2(dataQS(:,:,1)));
        str = [str ' f(x) = x log x'];
    case 3
        %(fx = y/ x^2)
        dataC(:,:,2) = (dataC(:,:,2) ./ power(dataC(:,:,1),2));
        dataQ(:,:,2) = (dataQ(:,:,2) ./ power(dataQ(:,:,1),2));
        dataQR(:,:,2) = (dataQR(:,:,2) ./ power(dataQR(:,:,1),2));
        dataQRS(:,:,2) = (dataQRS(:,:,2) ./ power(dataQRS(:,:,1),2));
        dataQS(:,:,2) = (dataQS(:,:,2) ./ power(dataQS(:,:,1),2));
        str = [str ' f(x) = x^2'];
    otherwise
        str = 'Invalid';
end
