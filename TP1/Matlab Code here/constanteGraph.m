function constanteGraph( dataC, dataQ, dataQR, dataQRS, dataQS, str, fx, saveName )
...
switch(fx)
    ...
    case 2
        %(fx = x log x)
        dataC(:,:,1) = dataC(:,:,1) .* log2(dataC(:,:,1));
        dataQ(:,:,1) = dataQ(:,:,1).* log2(dataQ(:,:,1));
        dataQR(:,:,1) = dataQR(:,:,1).*log2(dataQR(:,:,1));
        dataQRS(:,:,1) =dataQRS(:,:,1) .*log2(dataQRS(:,:,1));
        dataQS(:,:,1) = dataQS(:,:,1).*log2(dataQS(:,:,1));
        str = [str ' f(x) = x log x'];
    case 3
        %(fx = x^2)
        dataC(:,:,1) = power(dataC(:,:,1),2);
        dataQ(:,:,1) = power(dataQ(:,:,1),2);
        dataQR(:,:,1) = power(dataQR(:,:,1),2);
        dataQRS(:,:,1) = power(dataQRS(:,:,1),2);
        dataQS(:,:,1) = power(dataQS(:,:,1),2);
        str = [str ' f(x) = x^2'];
    otherwise
        str = 'bad case';
end
